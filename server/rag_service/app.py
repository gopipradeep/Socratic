    ...
    def llm_wrapper(prompt, api_key=None):
        ...
        for attempt in range(5): # Increased to 5 attempts
            try:
                ...
                # Basic check for 429/ResourceExhausted (string parsing as a fallback if types aren't available)
                if "429" in str(e) or "ResourceExhausted" in str(e) or "Too Many Requests" in str(e):
                    wait_time = (2 ** attempt) + random.uniform(0, 1)
                    logger.warning(f"LLM Rate Limit Hit (429). Retrying in {wait_time:.2f}s... (Attempt {attempt + 1}/5)")
                    time.sleep(wait_time)
                else:
                    logger.warning(f"LLM generation attempt {attempt + 1} failed: {e}")
                    if attempt == 4: raise # Raise on last attempt
...
@app.route('/socratic_chat', methods=['POST'])
def socratic_chat_route():
    data = request.get_json()
    if not data: return create_error_response("Request must be JSON", 400)
    ...
    try:
        ...
        from prompts import SOCRATIC_TUTOR_PROMPT_TEMPLATE
        prompt = SOCRATIC_TUTOR_PROMPT_TEMPLATE.format(user_input=user_input, chat_history=formatted_history)
        
        response_text = llm_wrapper(prompt, api_key)
        
        return jsonify({"reply": response_text}), 200
    except Exception as e:
        return create_error_response(f"Socratic chat failed: {str(e)}", 500)
...
def search_qdrant_documents():
    ...
    try:
        ...
        # --- FIX: Handle disabled vector service ---
        if vector_service:
            ...
            current_app.logger.warning("Vector service is disabled. Skipping Qdrant filter creation and search.")
            retrieved_docs, snippet_from_vector, docs_map = [], "", {}
        # --- END FIX ---
